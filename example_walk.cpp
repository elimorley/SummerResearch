...
void Custom::RobotControl() 
{
    ...
    //std::cout << "state =" << state.imu.gyroscope[3] << endl;
    printf("%d   %f\n", motiontime, state.imu.quaternion[2]);

    //printf("%d   %f\n", cmd.velocity[0], cmd.yawSpeed);
    //std::cout << "velocity =" << cmd.velocity[0];
    //std::cout << "          ";
    //std::cout << "yawspeed =" << cmd.yawSpeed << endl;
    ...
    if(motiontime > 2000){
        cmd.mode = 2;
        cmd.gaitType = 1; //gate 1 keeps it stationary in rotation
        //gate 2 allows it to roam around
        //cmd.velocity[0] = 0.4f; // -1  ~ +1
        // if you at cmd.velocity it creates an arc in motion
        cmd.yawSpeed = -1; // positve = counter  negative = clockwise
        //larger the value the quicker it turns
    ...
    //     //when velocity[0] positve = forward, negative = backwars
    //     //when velocity[1] postive = left, negative = right
    //     cmd.bodyHeight = 0.1;
    //     // printf("walk\n");
    //     }
